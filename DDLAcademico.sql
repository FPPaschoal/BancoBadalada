...
id_grade		INTEGER CONSTRAINT pk_tb_grades_id_grade PRIMARY KEY,
...
id_curso		VARCHAR(6)	CONSTRAINT pk_tb_cursos_id_curso PRIMARY KEY,
...
nm_departamento			VARCHAR(40)	CONSTRAINT nn_tb_departamentos_nm_depto NOT NULL,
									CONSTRAINT ck_tb_departamentos_nm_depto CHECK(nm_departamento = UPPER(nm_departamento)),
localizacao				VARCHAR(60) CONSTRAINT nn_tb_departamentos_localizacao NOT NULL,
									CONSTRAINT ck_tb_departamentos_localizacao CHECK (localizacao = UPPER(localizacao)),
id_gerente				INTEGER,
fg_ativo				BIT,
...
fg_ativo		BIT,
CONSTRAINT pk_tb_cursos_oferecidos_ PRIMARY KEY (id_curso, dt_inicio),
...
id_participante	INTEGER		CONSTRAINT nn_tb_matriculas_id_participantes NOT NULL,
...
CONSTRAINT ck_tb_historicos CHECK(dt_inicio < dt_final),
...
INSERT INTO tb_empregados(id_empregado,nm_empregado,iniciais_empregado,ds_cargo,id_gerente,dt_nascimento,salario,comissao,id_departamento,fg_ativo)
VALUES
...
INSERT INTO tb_departamentos(id_departamento, nm_departamento,
...
-- Elabore uma consulta para exebir o ID e a descrição de 
-- todos os cursos com duração de 4 dias.
SELECT id_curso, ds_curso
FROM tb_cursos
WHERE duracao = 4;

-- Ealbore uma consulta que liste todos os empregados ordenados 
-- pelo cargo e pela idade (do mais jovem ao mais velho)
SELECT *
FROM tb_empregados
ORDER BY ds_cargo, dt_nascimento DESC


-- Quais cursos foram realizados em Chicago e/ou em Seattle
SELECT * FROM tb_cursos_oferecidos
WHERE localizacao = 'Chicago' OR localizacao = 'Seattle'


-- Quais empregados participaram do curso java e do curso XML? (fornecer somente o codigo do empregado)
SELECT id_participante
FROM tb_matriculas
WHERE id_curso = 'JAV'
AND id_participante IN (SELECT id_participante
						FROM tb_matriculas
						WHERE id_curso = 'XML')

-- Liste os nomes e iniciais de todos os empregados, exceto do R.Jones
SELECT iniciais_empregado + ' ' + nm_empregado
FROM tb_empregados
WHERE iniciais_empregado + ' ' + nm_empregado <> 'R JONES'	

-- Encontre o ID, cargo e data de nascimento de todos os trainer e representantes de venda nascido antes de 1960
SELECT id_empregado, ds_cargo, dt_nascimento
FROM tb_empregados
WHERE (ds_cargo = 'TRAINER' OR ds_cargo = 'SALESREP') AND dt_nascimento < '1960-01-01'

--Liste os ids de todos os empregados que nao trabalham para o departamento de Training (id 20)
SELECT id_empregado
FROM tb_empregados
WHERE NOT(id_departamento = (SELECT id_departamento
                             FROM tb_departamentos
							 WHERE nm_departamento = 'TRAINING'))


--Liste os IDs de todos os empregados que não participam do curso de Java
SELECT id_empregado
FROM tb_empregados
WHERE NOT(id_empregado IN (SELECT id_participante FROM tb_matriculas WHERE id_curso = 'JAV'))

--Quais empregados tem subordinados
SELECT id_empregado, nm_empregado, iniciais_empregado
FROM tb_empregados
WHERE id_empregado IN (SELECT id_gerente FROM tb_empregados)


--Quais empregados não tem subordinados
SELECT id_empregado, nm_empregado, iniciais_empregado
FROM tb_empregados
WHERE NOT (id_empregado IN (SELECT id_empregado FROM tb_empregados
WHERE id_empregado IN (SELECT id_gerente FROM tb_empregados)))

SELECT * 
FROM tb_cursos_oferecidos
WHERE dt_inicio BETWEEN '1999-01-01' AND '1999-12-31' AND id_curso IN (SELECT id_curso FROM tb_cursos WHERE categoria = 'GEN')


-- Elabore uma consulta para recuperar o nome e iniciais de todos os empregados que já participaram de um curso ministrado por N. Smith
...
WHERE id_empregado IN (SELECT id_participante
					  ...
					  WHERE id_curso IN (SELECT id_curso
										 FROM tb_cursos_oferecidos
										 WHERE id_instrutor = (SELECT id_empregado
																FROM tb_empregados
																WHERE nm_empregado = 'SMITH' 
																AND iniciais_empregado = 'N'
        )
    )
);
